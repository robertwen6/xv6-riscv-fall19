...
bd_malloc(uint64 nbytes)
{
  ...
  bit_change(bd_sizes[k].alloc, blk_index(k, p));
  for(; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k-1);   // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    bit_change(bd_sizes[k-1].alloc, blk_index(k-1, p));
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_change(bd_sizes[k].alloc, bi);  // free p at size k
...
bd_mark(void *start, void *stop)
{
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for(; bi < bj; bi++) {
      ...
      bit_change(bd_sizes[k].alloc, bi);
    }
  }
}
...
int
bd_initfree_pair1(int k, int bi) {    //改!!!!!!!!!!!!!!!!!!!!!
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  if(bit_isset(bd_sizes[k].alloc, bi/2)) {   //alloc = 1
...
int
bd_initfree_pair2(int k, int bi) {    //改!!!!!!!!!!!!!!!!!!!!!
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  if(bit_isset(bd_sizes[k].alloc, bi/2)) {   //alloc = 1
    // one of the pair is free
...
bd_initfree(void *bd_left, void *bd_right) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {   // skip max size
    ...
    free += bd_initfree_pair1(k, left);
    if(right <= left)
      continue;
    free += bd_initfree_pair2(k, right);
  }
  return free;
}
...
bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    sz = sizeof(char)* ROUNDUP(NBLK(k), 16)/16;  //两个相邻块共用一个，空间减半
    bd_sizes[k].alloc = p;
